...
struct config::ConfigurationTraits<LogLevel> {
    static nlohmann::json ToJson(const LogLevel& value)
    {
        switch (value) {
        case LogLevel::Debug:
            return nlohmann::json("debug");
        case LogLevel::Info:
            return nlohmann::json("info");
        case LogLevel::Warning:
            return nlohmann::json("warning");
        case LogLevel::Error:
...
struct config::ConfigurationTraits<DatabaseConfig> {
    ...
    {
        return !value.host.empty() && value.port > 0 && value.port < 65536 && !value.username.empty();
    }
    ...
    static std::string GetValidationError(const DatabaseConfig& value)
    {
        ...
        if (value.port <= 0 || value.port >= 65536)
            return "Database port must be between 1-65535";
...
{
    if (str == "application_name")
        return AppConfig::ApplicationName;
    if (str == "log_level")
        return AppConfig::LogLevel;
    if (str == "max_threads")
        return AppConfig::MaxThreads;
    if (str == "database_config")
...
int main()
{
    ...
    std::cout << "  Updated Log Level: " << config::ConfigurationTraits<LogLevel>::ToString(appConfig.GetSetting<AppConfig::LogLevel>().Value()) << "\n";
    std::cout << "  Updated Database: " << config::ConfigurationTraits<DatabaseConfig>::ToString(appConfig.GetSetting<AppConfig::DatabaseConfig>().Value()) << "\n\n";
